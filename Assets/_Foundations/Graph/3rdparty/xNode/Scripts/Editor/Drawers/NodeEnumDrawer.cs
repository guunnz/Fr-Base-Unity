using System;
using Graph._3rdparty.xNode.Scripts.Attributes;
using UnityEditor;
using UnityEngine;

namespace Graph._3rdparty.xNode.Scripts.Editor.Drawers
{
    ...
    public class NodeEnumDrawer : PropertyDrawer
    {
        ...
        public static void EnumPopup(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            // Display dropdown
            if (EditorGUI.DropdownButton(position, new GUIContent(enumName),
                    FocusType.Passive)) // Position is all wrong if we show the dropdown during the node draw phase.
