...
using System.Linq;
using Graph._3rdparty.xNode.Scripts.Editor.Internal;
using UnityEditor;
using UnityEngine;
...
namespace Graph._3rdparty.xNode.Scripts.Editor
{
    public partial class NodeEditorWindow
    {
        ...
        public void RemoveSelectedNodes()
        {
            ...
            for (var i = 0; i < selectedReroutes.Count; i++) selectedReroutes[i].RemovePoint();
            selectedReroutes.Clear();
        ...
        private void InsertDuplicateNodes(Node[] nodes, Vector2 topLeft)
        {
            ...
            for (var i = 0; i < nodes.Length; i++)
            {
                ...
                foreach (var port in srcNode.Ports)
                    ...
                    {
                        var inputPort = port.direction == NodePort.IO.Input ? port : port.GetConnection(c);
                        var outputPort = port.direction == NodePort.IO.Output ? port : port.GetConnection(c);
        ...
        public void DrawDraggedConnection()
        {
            if (IsDraggingPort)
            {
                ...
                for (var i = 0; i < draggedOutputReroutes.Count; i++) gridPoints.Add(draggedOutputReroutes[i]);
