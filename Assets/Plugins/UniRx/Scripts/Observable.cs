...
namespace UniRx
{
    ...
    public static partial class Observable
    {
        ...
        public static IObservable<TR> Select<T, TR>(this IObservable<T> source, Func<TR> selector)
        {
            return Select(source, _ => selector());
        }
        ...
        public static IObservable<T> DoMany<T>(this IObservable<T> source, Func<T, IObservable<T>> selector)
        {
            return source.SelectMany(a => selector(a).Select(_ => a));
        }
        ...
        public static IObservable<bool> If(this IObservable<bool> source, Action action)
        {
            return source.Do(condition =>
            {
                if (condition)
                {
                    action?.Invoke();
        ...
        public static IObservable<bool> Else(this IObservable<bool> source, Action action)
        {
            return source.Do(condition =>
            {
                if (!condition)
        ...
        public static IObservable<T> Do<T>(this IObservable<T> source, Action onNext)
        {
            return new DoObservable<T>(source, _ => onNext(), Stubs.Throw, Stubs.Nop);
        }
