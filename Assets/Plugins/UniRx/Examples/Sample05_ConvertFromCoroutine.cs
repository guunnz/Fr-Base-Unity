...
namespace UniRx.Examples
{
    public class Sample05_ConvertFromCoroutine
    {
        ...
        private static IEnumerator GetWWWCore(string url, IObserver<string> observer,
            CancellationToken cancellationToken)
        {
            var www = new WWW(url);
            while (!www.isDone && !cancellationToken.IsCancellationRequested) yield return null;
