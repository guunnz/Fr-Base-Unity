...
namespace UniRx.Examples
{
    public class Sample13_ToDoApp : MonoBehaviour
    {
        ...
        private readonly ReactiveCollection<GameObject> toDos = new ReactiveCollection<GameObject>();

        private void Start()
        {
            // merge Button click and push enter key on input field.
            var submit = AddButton.OnClickAsObservable().Select(_ => ToDoInput.text)
                .Merge(ToDoInput.OnEndEditAsObservable().Where(_ => Input.GetKeyDown(KeyCode.Return)));
