...
namespace Unity.VectorGraphics
{
    public static partial class VectorUtils
    {
        ...
        /// <param name="customPivot">
        ///     If alignment is <see cref="Alignment.Custom" />, customPivot is used to compute the sprite
        ///     origin
        /// </param>
        ...
        /// <returns>
        ///     A new Sprite containing the provided geometry. The Sprite may have a texture if the geometry has any texture
        ///     and/or gradients
        /// </returns>
        public static Sprite BuildSprite(List<Geometry> geoms, float svgPixelsPerUnit, Alignment alignment,
            Vector2 customPivot, ushort gradientResolution, bool flipYAxis = false)
        {
            return BuildSprite(geoms, Rect.zero, svgPixelsPerUnit, alignment, customPivot, gradientResolution,
                flipYAxis);
        }
        ...
        ///     and/or gradients
        /// </returns>
        public static Sprite BuildSprite(List<Geometry> geoms, Rect rect, float svgPixelsPerUnit, Alignment alignment,
            Vector2 customPivot, ushort gradientResolution, bool flipYAxis = false)
        {
            // Generate atlas
            ...
            FillVertexChannels(geoms, 1.0f, texAtlas != null, out vertices, out indices, out colors, out uvs,
                out settingIndices, flipYAxis);

            var texture = texAtlas != null ? texAtlas.Texture : null;
            ...
            if (colors != null)
            {
                ...
                {
                    sprite.SetVertexAttribute(VertexAttribute.Color, nativeColors);
                }
            }
            ...
            if (uvs != null)
            {
                ...
                {
                    sprite.SetVertexAttribute(VertexAttribute.TexCoord0, nativeUVs);
                }

                using (var nativeSettingIndices = new NativeArray<Vector2>(settingIndices.ToArray(), Allocator.Temp))
                {
                    sprite.SetVertexAttribute(VertexAttribute.TexCoord2, nativeSettingIndices);
                }
            }
        ...
        {
            var hasUVs = geoms.FirstOrDefault(g => g.UVs != null) != null;
            ...
            List<Vector2> uvs;
            List<Vector2> settingIndices;
            FillVertexChannels(geoms, svgPixelsPerUnit, hasUVs, out vertices, out indices, out colors, out uvs,
                out settingIndices, flipYAxis);
        ...
        private static void FillVertexChannels(List<Geometry> geoms, float pixelsPerUnit, bool hasUVs,
            out List<Vector2> vertices, out List<ushort> indices, out List<Color> colors, out List<Vector2> uvs,
            out List<Vector2> settingIndices, bool flipYAxis)
        {
            ...
            indices = new List<ushort>(totalIndices);
            ...
            foreach (var geom in geoms)
            {
                ...
                var indexEnd = indexStart + geom.Indices.Length;

                var vertexCount = vertices.Count;
                indices.AddRange(geom.Indices.Select(x => (ushort) (x + vertexCount)));
                ...
                Debug.Assert(uvs == null || geom.UVs != null);
                if (uvs != null)
                {
                    uvs.AddRange(geom.UVs);
                    for (var i = 0; i < geom.UVs.Length; i++)
        ...
        private static void FlipRangeIfNecessary(List<Vector2> vertices, List<ushort> indices, int indexStart,
            int indexEnd, bool flipYAxis)
        {
            ...
            var shouldFlip = false;
            for (var i = indexStart; i < indexEnd - 2; i += 3)
            {
                var v0 = (Vector3) vertices[indices[i]];
                var v1 = (Vector3) vertices[indices[i + 1]];
                var v2 = (Vector3) vertices[indices[i + 2]];
        ...
        internal static void RenderFromArrays(Vector2[] vertices, ushort[] indices, Vector2[] uvs, Color[] colors,
        ...
        public static void RenderSprite(Sprite sprite, Material mat, bool clear = true)
        {
            ...
            var pixelsToUnits = sprite.rect.width / sprite.bounds.size.x;
        ...
        ///     When true, expand the edges to avoid a dark banding effect caused by filtering. This is
        ///     slower to render and uses more graphics memory.
        /// </param>
