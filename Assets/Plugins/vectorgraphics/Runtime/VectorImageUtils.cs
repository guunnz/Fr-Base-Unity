...
namespace Unity.VectorGraphics
{
    ...
    {
        internal static void MakeVectorImageAsset(IEnumerable<VectorUtils.Geometry> geoms, uint rasterSize,
            out Object outAsset, out Texture2D outTexAtlas)
        {
            var atlas = VectorUtils.GenerateAtlas(geoms, rasterSize, false, false);
            if (atlas != null)
                VectorUtils.FillUVs(geoms, atlas);

            var hasTexture = atlas != null && atlas.Texture != null;
            outTexAtlas = hasTexture ? atlas.Texture : null;

            var vertices = new List<InternalBridge.VectorImageVertexBridge>(100);
            var indices = new List<ushort>(300);
            ...
            if (hasTexture && atlas != null && atlas.Entries != null && atlas.Entries.Count > 0)
            {
                ...
                settings.Add(new InternalBridge.GradientSettingsBridge
                {
                    ...
                    location = new RectInt((int) entry.Position.x, (int) entry.Position.y, (int) entry.Size.x,
                        (int) entry.Size.y)
                });
            }
            ...
            foreach (var geom in geoms)
            {
                ...
                indices.AddRange(geom.Indices.Select(i => (ushort) (i + count)));

                if (atlas != null && atlas.Entries != null && atlas.Entries.Count > 0)
                {
                    VectorUtils.PackRectItem entry;
                    if (geom.Fill == null || !fillEntries.TryGetValue(geom.Fill, out entry) ||
                        writtenSettings.Contains(entry.SettingIndex))
                        continue;
        ...
        internal static Texture2D RenderVectorImageToTexture2D(Object o, int width, int height, Material mat,
            int antiAliasing = 1)
        {
            if (o == null)
                return null;
            ...
            if (InternalBridge.GetDataFromVectorImage(o, ref vertices, ref indices, ref uvs, ref colors,
                ref settingIndices, ref settings, ref atlas, ref size))
            {
                ...
                VectorUtils.RenderFromArrays(vertices, indices, uvs, colors, settingIndices, atlasWithEncodedSettings,
                    mat);
                Object.DestroyImmediate(atlasWithEncodedSettings);
            }
            else
            {
        ...
        private static Texture2D BuildAtlasWithEncodedSettings(InternalBridge.GradientSettingsBridge[] settings,
            Texture2D atlas)
        {
            var oldActive = RenderTexture.active;

            var width = atlas.width + 3;
            var height = Math.Max(settings.Length, atlas.height);
            ...
            for (var i = 0; i < settings.Length; ++i)
            {
                ...
                if (g.gradientType == InternalBridge.GradientTypeBridge.Radial)
                {
                    ...
                    VectorUtils.WriteRawFloat4Packed(rawSettingsTex, (float) g.gradientType / 255,
                        (float) g.addressMode / 255, focus.x, focus.y, destX++, destY);
                }
                else
                {
                    VectorUtils.WriteRawFloat4Packed(rawSettingsTex, 0.0f, (float) g.addressMode / 255, 0.0f, 0.0f,
                        destX++, destY);
                }
                ...
                size.x -= 1;
                size.y -= 1;
                VectorUtils.WriteRawInt2Packed(rawSettingsTex, pos.x + 3, pos.y, destX++, destY);
                VectorUtils.WriteRawInt2Packed(rawSettingsTex, size.x, size.y, destX++, destY);
            }
