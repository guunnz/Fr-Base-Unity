...
namespace Unity.VectorGraphics
{
    ...
    public static partial class VectorUtils
    {
        ...
        public static void MakeRectangleShape(Shape rectShape, Rect rect, Vector2 radiusTL, Vector2 radiusTR,
            Vector2 radiusBR, Vector2 radiusBL)
        {
        ...
        public static BezierPathSegment[] MakePathLine(Vector2 from, Vector2 to)
        {
            return new[]
            {
                new BezierPathSegment
                    {P0 = from, P1 = (to - from) / 3.0f + from, P2 = (to - from) * 2.0f / 3.0f + from},
        ...
        internal static BezierSegment MakeArcQuarter(Vector2 center, float startAngleRads, float sweepAngleRads)
        {
            ...
            var s = Mathf.Sin(sweepAngleRads);
            var c = Mathf.Cos(sweepAngleRads);
            var m = Matrix2D.RotateLH(startAngleRads);
            m.m02 = center.x;
            m.m12 = center.y;
            var f = 0.551915024494f;
            return new BezierSegment
            {
        ...
        public static BezierPathSegment[] MakeArc(Vector2 center, float startAngleRads, float sweepAngleRads,
            float radius)
        {
            ...
            var endQuadrant = QuadrantAtAngle(sweepAngleRads);

            for (var quadrant = 0; quadrant <= endQuadrant; ++quadrant)
            {
        ...
        public static void Bounds(BezierSegment segment, out Vector2 min, out Vector2 max)
        {
            ...
            var a = 3.0f * segment.P3 - 9.0f * segment.P2 + 9.0f * segment.P1 - 3.0f * segment.P0;
            var b = 6.0f * segment.P2 - 12.0f * segment.P1 + 6.0f * segment.P0;
            var c = 3.0f * segment.P1 - 3.0f * segment.P0;

            var solutions = new float[4];
        ...
        public static Vector2 EvalFull(BezierSegment segment, float t, out Vector2 tangent)
        {
            var t2 = t * t;
            var t3 = t2 * t;
            var C1 = segment.P3 - 3.0f * segment.P2 + 3.0f * segment.P1 - segment.P0;
            var C2 = 3.0f * segment.P2 - 6.0f * segment.P1 + 3.0f * segment.P0;
            var C3 = 3.0f * segment.P1 - 3.0f * segment.P0;
            var C4 = segment.P0;
        ...
        {
            var pos = EvalFull(segment, t, out tangent);
        ...
        /// <param name="closed">
        ///     A boolean indicating if the length of the segment joining the first and last points should be
        ///     computed
        /// </param>
        ...
        {
            var segmentLengths = new float[segments.Count - 1 + (closed ? 1 : 0)];
            var i = 0;
        ...
        public static float SegmentLength(BezierSegment segment, float precision = 0.001f)
        {
            ...
            {
                var steps = Math.Min(100, (int) (1.0f / precision));
            ...
            while ((tmax = AdaptiveQuadraticApproxSplitPoint(segment, precision)) < 1.0f)
            {
                ...
                var midPointLength = MidPointQuadraticApproxLength(b1);
        ...
        {
            var quadraticApproxDist =
                (segment.P3 - 3.0f * segment.P2 + 3.0f * segment.P1 - segment.P0).magnitude * 0.5f;
        ...
        private static float MidPointQuadraticApproxLength(BezierSegment segment)
        {
            ...
            if (A1 != Vector2.zero)
            {
                ...
                var l1 = q / (8.0f * Math.Pow(c, 1.5f)) * (Math.Log(2.0f * Math.Sqrt(c * sumCBA) + twoCpB) -
                                                           Math.Log(2.0f * Math.Sqrt(c * a) + b));
                return (float) (l0 + l1);
            }
        ...
        public static BezierSegment TransformSegment(BezierSegment segment, Vector2 translation, float rotation,
            Vector2 scaling)
        {
        ...
        public static BezierPathSegment[] TransformBezierPath(BezierPathSegment[] path, Vector2 translation,
            float rotation, Vector2 scaling)
        {
            var m = Matrix2D.RotateLH(rotation);
        ...
        private static IEnumerable<SceneNodeWorldTransform> WorldTransformedSceneNodes(SceneNode child,
            Dictionary<SceneNode, float> nodeOpacities, SceneNodeWorldTransform parent)
        {
        ...
        public static IEnumerable<SceneNodeWorldTransform> WorldTransformedSceneNodes(SceneNode root,
        ...
        /// <param name="line2Pt2">The second point of the second line</param>
        /// <returns>The intersection point, or (float.PositiveInfinity, float.PositiveInfinity) if the lines are parallel</returns>
        public static Vector2 IntersectLineSegments(Vector2 line1Pt1, Vector2 line1Pt2, Vector2 line2Pt1,
            Vector2 line2Pt2)
        {
            var a1 = (line1Pt1.x - line2Pt2.x) * (line1Pt2.y - line2Pt2.y) -
                     (line1Pt1.y - line2Pt2.y) * (line1Pt2.x - line2Pt2.x);
            var a2 = (line1Pt1.x - line2Pt1.x) * (line1Pt2.y - line2Pt1.y) -
                     (line1Pt1.y - line2Pt1.y) * (line1Pt2.x - line2Pt1.x);
            if (a1 * a2 <= 0.0f)
            {
                var a3 = (line2Pt1.x - line1Pt1.x) * (line2Pt2.y - line1Pt1.y) -
                         (line2Pt1.y - line1Pt1.y) * (line2Pt2.x - line1Pt1.x);
                var a4 = a3 + a2 - a1;
                if (a3 * a4 <= 0.0f)
                {
                    var t = a3 / (a3 - a4);
        ...
        private static float[] CubicRoots(double a, double b, double c, double d)
        {
            ...
            t.RemoveAll(x => Math.Abs(x + 1.0) < Epsilon);
        ...
        {
            return (bs.P0 - bs.P1).sqrMagnitude <= Epsilon && (bs.P0 - bs.P2).sqrMagnitude <= Epsilon &&
                   (bs.P0 - bs.P3).sqrMagnitude <= Epsilon;
        }
...
#pragma warning restore 612, 618
    ...
    internal class PathDistanceForwardIterator
    {
        ...
        public PathDistanceForwardIterator(IList<BezierPathSegment> pathSegments, bool closed, float maxCordDeviationSq,
            float maxTanAngleDevCosine, float stepSizeT)
        {
            if (pathSegments.Count < 2)
                throw new Exception("Cannot iterate a path with no segments in it");

            Segments = closed && !VectorUtils.PathEndsPerfectlyMatch(pathSegments)
                ? new BezierLoop(pathSegments)
                : pathSegments;
            Closed = closed;
            ...
            currentBezSeg = new BezierSegment
                {P0 = pathSegments[0].P0, P1 = pathSegments[0].P1, P2 = pathSegments[0].P2, P3 = pathSegments[1].P0};
        ...
        public float LengthSoFar { get; private set; }

        public float SegmentLengthSoFar { get; private set; }

        public bool Ended => CurrentT == 1.0f && CurrentSegment + 1 == Segments.Count - 1;

        private float PointToLineDistanceSq(Vector2 point, Vector2 lineStart, Vector2 lineEnd)
        {
            var lineMagSq = (lineEnd - lineStart).sqrMagnitude;
            if (lineMagSq < VectorUtils.Epsilon)
                return (point - lineStart).sqrMagnitude;
            var num = (lineEnd.y - lineStart.y) * point.x - (lineEnd.x - lineStart.x) * point.y +
        ...
        public Result AdvanceBy(float units, out float unitsRemaining)
        {
            ...
            var t = CurrentT;
            var currentTPosition = lastPointEval;
            for (;;)
            {
                var nextT = Mathf.Min(t + stepSizeT, 1.0f);
                var tangent = Vector2.zero;
                var point = needTangentsDuringEval
                    ? VectorUtils.EvalFull(currentBezSeg, nextT, out tangent)
                    : VectorUtils.Eval(currentBezSeg, nextT);

                var generateStepHere = false;
                if (needTangentsDuringEval)
                {
                    var tangentDiffCosAngle = Vector2.Dot(tangent, currentTTangent);
                    generateStepHere = tangentDiffCosAngle < maxTanAngleDevCosine;
                }

                if (!generateStepHere && maxCordDeviationSq != float.MaxValue)
                {
                    var firstPoint = currentTPosition;
                    var distPtToFirstSq = (point - firstPoint).sqrMagnitude;
                    if (distPtToFirstSq > VectorUtils.Epsilon)
                    {
                        var secondPoint = VectorUtils.Eval(currentBezSeg,
                            Mathf.Min((nextT - CurrentT) * 2.0f + CurrentT, 1.0f));
                        var midPointDistSq = PointToLineDistanceSq(point, firstPoint, secondPoint);
                        generateStepHere = midPointDistSq >= maxCordDeviationSq;
                    }
                }

                var dist = (point - lastPointEval).magnitude;
                if (dist > unitsRemaining)
                {
                    nextT = t + stepSizeT *
                        (unitsRemaining / dist); // A linear approximation, not too bad for small step sizes
                    dist = unitsRemaining;
        ...
        {
            private readonly IList<BezierPathSegment> OpenPath;
    ...
    internal class PathPatternIterator
    {
        ...
        private readonly float patternLength;
        private readonly float patternOffset;
        ...
        public PathPatternIterator(float[] pattern, float patternOffset = 0.0f)
        {
            ...
            else IsSolid = IsSolidAt(0.0f, out currentSegment, out segmentLength);
        }
        ...
        public bool IsSolidAt(float unitsFromPathStart, out int patternSegmentIndex, out float patternSegmentLength)
        {
            ...
            var hops = (int) (Mathf.Abs(unitsFromPathStart) / patternLength);
