...
namespace Unity.VectorGraphics
{
    ...
    {
        internal static BezierPathSegment[] BuildEllipsePath(Vector2 p0, Vector2 p1, float rotation, float rx, float ry,
            bool largeArc, bool sweep)
        {
            ...
            float adjustedRx;
            float adjustedRy;
            ComputeEllipseParameters(p0, p1, rotation, rx, ry, largeArc, sweep, out c, out theta1, out sweepTheta,
                out adjustedRx, out adjustedRy);
        ...
        private static void ComputeEllipseParameters(Vector2 p0, Vector2 p1, float phi, float rx, float ry, bool fa,
            bool fs, out Vector2 c, out float theta1, out float sweepTheta, out float adjustedRx, out float adjustedRy)
        {
            ...
            theta1 = Vector2.SignedAngle(new Vector2(1, 0), new Vector2((pp.x - cp.x) / rx, (pp.y - cp.y) / ry)) %
                     360.0f;
            sweepTheta = Vector2.SignedAngle(new Vector2((pp.x - cp.x) / rx, (pp.y - cp.y) / ry),
                new Vector2((-pp.x - cp.x) / rx, (-pp.y - cp.y) / ry));
