...
namespace Unity.VectorGraphics.Editor
{
    ...
    {
        private readonly GUIContent[] m_AlignmentOptions =
        {
        ...
        private readonly GUIContent m_GradientResolutionText = new GUIContent("Gradient Resolution",
            "Size of each rasterized gradient in pixels. Higher values consume memory but result in more accurate gradients.");

        private readonly GUIContent m_KeepTextureAspectRatioText = new GUIContent("Keep Aspect Ratio");

        private readonly GUIContent m_MaxCordDeviationEnabledText = new GUIContent("Max Cord Enabled",
            "Enables the \"max cord deviation\" tessellation test.");

        private readonly GUIContent m_MaxCordDeviationText = new GUIContent("Max Cord Deviation",
            "Distance on the cord to a straight line between two points after which more tessellation will be generated.");

        private readonly GUIContent m_MaxTangentAngleEnabledText = new GUIContent("Max Tangent Enabled",
            "Enables the \"max tangent angle\" tessellation test.");

        private readonly GUIContent m_MaxTangentAngleText = new GUIContent("Max Tangent Angle",
            "Max tangent angle (in degrees) after which more tessellation will be generated.");

        private readonly GUIContent m_PixelsPerUnitText = new GUIContent("Pixels Per Unit",
            "How many pixels in the SVG correspond to one unit in the world.");

        private readonly GUIContent m_PreserveSVGImageAspectText = new GUIContent("Preserve Aspect");

        private readonly GUIContent m_ResolutionMultiplierText = new GUIContent("Zoom Factor",
            "Target zoom factor for which the SVG asset should not look tessellated.");
        ...
        private readonly GUIContent m_SamplingStepDistanceText = new GUIContent("Sampling Steps",
            "Number of samples evaluated on paths. Higher values give more accurate results (but takes longer).");

        private readonly GUIContent[] m_SettingOptions =
        {
        ...
        private readonly GUIContent m_StepDistanceText = new GUIContent("Step Distance",
            "Distance at which vertices will be generated along the paths. Lower values will result in a more dense tessellation.");

        private readonly GUIContent m_SVGTypeText =
            new GUIContent("Generated Asset Type", "How the SVG file will be imported.");

        private readonly GUIContent[] m_TargetResolutionOptions =
        {
        ...
        private readonly GUIContent m_TargetResolutionText = new GUIContent("Target Resolution",
            "Target resolution below which the sprite will not look tessellated.");

        private readonly GUIContent m_TexturedSpriteMeshTypeText =
            new GUIContent("Mesh Type", "Type of the sprite mesh to generate.");

        private readonly GUIContent m_TextureSizeText =
            new GUIContent("Texture Size", "The size of the generated texture.");

        private readonly GUIContent m_UseSVGPixelsPerUnitText = new GUIContent("Use SVG Pixels Per Unit",
            "When set, the \"Pixels Per Unit\" value will be applied relative to the SVG asset size instead of the texture size.");

        private readonly GUIContent m_ViewportOptionsText = new GUIContent("Viewport Options",
            "Viewport options to use while importing the SVG document");
...
#endif
            new GUIContent("UI SVGImage",
                "A tessellated sprite with \"infinite\" resolution, compatible with the UI canvas masking system.")
        };
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(m_SVGType.hasMultipleDifferentValues ||
            ...
            if (!m_AdvancedMode.hasMultipleDifferentValues)
            {
                ...
                else
                {
                    ...
                    var resolutionIndex = EditorGUILayout.Popup(m_TargetResolutionText,
                        m_PredefinedResolutionIndex.intValue, m_TargetResolutionOptions);
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        if (m_PredefinedResolutionIndex.intValue != (int) SVGImporter.PredefinedResolution.Custom)
                            m_TargetResolution.intValue =
                                TargetResolutionFromPredefinedValue(
                                    (SVGImporter.PredefinedResolution) m_PredefinedResolutionIndex.intValue);
                    }

                    if (!m_PredefinedResolutionIndex.hasMultipleDifferentValues &&
                        m_PredefinedResolutionIndex.intValue == (int) SVGImporter.PredefinedResolution.Custom)
            ...
            if (!m_SVGType.hasMultipleDifferentValues && (m_SVGType.intValue == (int) SVGType.TexturedSprite ||
                                                          m_SVGType.intValue == (int) SVGType.Texture2D))
            {
                ++EditorGUI.indentLevel;
                ...
                {
                    IntPopup(m_TexturedSpriteMeshType, m_TexturedSpriteMeshTypeText, texturedSpriteMeshTypeOptions,
                        texturedSpriteMeshTypeValues);
        ...
        protected override void Apply()
        {
            ...
            foreach (var target in targets)
            {
                ...
                svgImporter.GradientResolution =
                    Math.Min((ushort) 4096, Math.Max((ushort) 2, svgImporter.GradientResolution));
                ...
                svgImporter.TargetResolution = Mathf.Max(1, svgImporter.TargetResolution);
        ...
        private void IntField(SerializedProperty prop, GUIContent label, params GUILayoutOption[] options)
        {
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = prop.hasMultipleDifferentValues;
            var value = EditorGUILayout.IntField(label, prop.intValue, options);
            EditorGUI.showMixedValue = false;
            if (EditorGUI.EndChangeCheck())
                prop.intValue = value;
        }
        ...
        private void IntPopup(SerializedProperty prop, GUIContent label, GUIContent[] displayedOptions)
        {
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = prop.hasMultipleDifferentValues;
            var value = EditorGUILayout.Popup(label, prop.intValue, displayedOptions);
            EditorGUI.showMixedValue = false;
            if (EditorGUI.EndChangeCheck())
                prop.intValue = value;
        }
        ...
        private void IntPopup(SerializedProperty prop, GUIContent label, GUIContent[] displayedOptions, int[] options)
        {
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = prop.hasMultipleDifferentValues;
            var value = EditorGUILayout.IntPopup(label, prop.intValue, displayedOptions, options);
            EditorGUI.showMixedValue = false;
            if (EditorGUI.EndChangeCheck())
                prop.intValue = value;
        }
        ...
        private void BoolToggle(SerializedProperty prop, GUIContent label)
        {
            EditorGUI.BeginChangeCheck();
            EditorGUI.showMixedValue = prop.hasMultipleDifferentValues;
            var value = EditorGUILayout.Toggle(label, prop.boolValue);
            EditorGUI.showMixedValue = false;
            if (EditorGUI.EndChangeCheck())
        ...
        public override void OnPreviewGUI(Rect r, GUIStyle background)
        {
            ...
            if (sprite == null)
            {
                ...
                if (InternalBridge.GetDataFromVectorImage(assetTarget, ref vertices, ref indices, ref uvs, ref colors,
                    ref settingIndices, ref settings, ref atlas, ref size))
                {
            ...
            var zoomLevel = Mathf.Min(r.width / sourceSize.x, r.height / sourceSize.y);
            var wantedRect = new Rect(r.x, r.y, sourceSize.x * zoomLevel, sourceSize.y * zoomLevel);
        ...
        internal static Texture2D BuildPreviewTexture(Object obj, int width, int height)
        {
            Texture2D previewTex = null;
            ...
            else
            {
                ...
                var size = Vector2.zero;
                InternalBridge.GradientSettingsBridge[] settings = null;
                if (InternalBridge.GetDataFromVectorImage(obj, ref vertices, ref indices, ref uvs, ref colors,
                    ref settingIndices, ref settings, ref atlas, ref size))
                {
        ...
        public override string GetInfoString()
        {
            ...
            var indexCount = sprite.triangles.Length;

            var stats = "" + vertexCount + " Vertices (Pos";

            var vertexSize = sizeof(float) * 2;
