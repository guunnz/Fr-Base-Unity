...
namespace Unity.VectorGraphics.Editor
{
    ...
    {
        private readonly SVGImporter m_Importer;
    ...
    internal class SVGTextureDataProvider : SVGDataProviderBase, ITextureDataProvider
    {
        ...
        public Texture2D texture
        {
            get
            {
                if (GetImporter().SvgType == SVGType.TexturedSprite) return GetTexture2D();
        ...
        public Texture2D previewTexture
        {
            get
            {
                if (GetImporter().SvgType == SVGType.TexturedSprite) return texture;
                ...
                if (m_PreviewTexture == null)
                {
                    ...
                    m_PreviewTexture = VectorUtils.RenderSpriteToTexture2D(sprite, (int) (size.x * m_TextureScale),
                        (int) (size.y * m_TextureScale), mat, 4);
        ...
        public Texture2D GetReadableTexture2D()
        {
            if (m_ReadableTexture == null)
            {
                if (GetImporter().SvgType == SVGType.VectorSprite)
                {
                    var sprite = GetSprite();
                    var size = (Vector2) sprite.bounds.size * sprite.pixelsPerUnit;
                    m_ReadableTexture = VectorUtils.RenderSpriteToTexture2D(sprite, (int) size.x, (int) size.y,
                        SVGImporter.CreateSVGSpriteMaterial(sprite), 4);
                }
                else
                {
                    return GetTexture2D();
